...
export function isWalkable(cx: number, cz: number): boolean {
  if (cx < 0 || cz < 0 || cx >= COLS || cz >= ROWS) return false
  const ch = MAP[cz][cx]
  return ch === '.' || ch === 'p' || ch === 'c'
}

export function tileToWorld(cx: number, cz: number): THREE.Vector3 {
...
function building(w: number, d: number, rng: Rng): THREE.Group {
...
function oliveTree(rng: Rng): THREE.Group {
  const g = new THREE.Group()
...
export function buildTown(scene: THREE.Scene): void {
  ...
  const used: boolean[][] = MAP.map(row => row.split('').map(() => false))
  for (let z = 0; z < ROWS; z++) {
    for (let x = 0; x < COLS; x++) {
